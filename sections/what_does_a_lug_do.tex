...
\begin{itemize}
\item ترویج \LRE{(advocacy)}
%\item advocacy
\item آموزش \LRE{(education)}
%\item education
\item پشتیبانی \LRE{(support)}
%\item support
\item اجتماعی‌شدن \LRE{(socialising)}
%\item socialising
\end{itemize}
...
\subsection{ترویج گنو/لینوکس \LRE{(advocacy)}}
%\subsection{GNU/Linux advocacy}


تمایل به ترویج \LRE{(advocacy)}
\footnote{
معنای دقیق \LRE{advocacy} ترویج نیست و
معادل‌های مختلفی برای ترجمه آن می‌توان استفاده کرد که البته
هیچکدام از آنها نمی‌تواند بطور دقیق معنای آنرا برساند. \LRE{advocacy}
در لغت می‌تواند به معنای طرفداری، ترویج، مدافعه و وکالت باشد.
بطور کلی \LRE{advocacy} به معنای پشتیبانی از یک ایده می‌باشد.
درطول این مستند معادل‌هایی مانند ترویج یا طرفداری را خواهید دید
و منظور ما از تمام آنها \LRE{advocacy} خواهد بود.
}
...
\emph{راهنمای ترویج لینوکس}\footnote{\texttt{\url{http://www.tldp.org/HOWTO/Advocacy.html}}}
در
\emph{پروژه مستندسازی لینوکس}\footnote{\texttt{\url{http://www.tldp.org/}}}
موجود می‌باشد و پیشنهادهای بسیار کمک کننده ای ارائه می‌دهد.
پروژه دیگری مشابه پروژه مستندسازی لینوکس، به نام 
\emph{\url{Linuxmanship}}\footnote{\texttt{\url{http://zgp.org/~dmarti/linuxmanship/}}}
وجود دارد که کار
\emph{دون مارتی}\footnote{\LRE{Don Marti}}
می‌باشد.
...
\subsection{محدودیت‌های ترویج (\LRE{advocacy})}
...
گوش بدهد. (همانطور که \LRE{Eric S. Raymond}
\emph{می‌گوید}\footnote{\texttt{\url{http://web.archive.org/web/20120131000749/http://www.itworld.com/print/36449}}}
،
\emph{
به منافع و علاقمندی‌های مشتری خود توجه کنید، نه خودتان.
}\footnote{Appeal to the prospect's interests and values, not to yours}
اگر شخصی ستاپ سیستم‌عامل انحصاری که درحال حاضر دارد را می‌پسندد
و آنرا دوست دارد، ترویج تنها وقت شما و او را هدر می‌دهد.
اگر نیازهای او کاملا با \LRE{MS-Project}، \LRE{MS-Visio} و
\emph{گروه‌افزار اوت‌لوک/استک‌اکسچنج}\footnote{\LRE{Outlook/Exchange groupware}}
...
آنها هرگز کسی را ندیده‌اند که بدون درنظر داشتن منفعت، آزاد
...
در عین حال مطمئن باشید که درگیر کلیشه‌های طرفداری بی‌مورد از یک سیستم عامل نشده‌اید.
تحمیل نظراتتان به کسی بدون اینکه او بخواهد، کاملاً بی‌ادبانه و توهین‌آمیز است.
. چنین تلاشی نه‌تنها بی‌فایده است، بلکه ممکن است اثر معکوس داشته باشد.
...
\emph{«ارزش استفاده»}\footnote{\texttt{\url{https://en.wikipedia.org/wiki/Use_value}}}
...
\LRE{Caldera Network Desktop}
(نام اولیه توزیع گنو/لینوکسی که توسعه داده بود) در پروژه
سیستم‌عامل دسکتاپ «\LRE{Corsair}» توسط شرکت \LRE{Novell} شنیدم.
پس از بررسی مدیران اجرایی و فنی آنها متوجه شدند که افسران شرکت‌ها
از هرچه می‌توانند به رایگان دریافت کنند ناراضی هستند. سپس \LRE{Caldera}
...
از این دیدگاه محتاط بودن درمورد هزینه‌ها و دشواری‌های توسعه گنو/لینوکس
به جذابیت آن کمک می‌کند و اعتبار شما را به‌عنوان یک سخنران حفظ می‌کند.
...
\emph{پایگاه داده تحلیلی}\footnote{\texttt{\url{https://en.wikipedia.org/wiki/Data_warehouse}}}،
\emph{مراکز تماس}\footnote{\texttt{\url{https://en.wikipedia.org/wiki/Call_centre}}}،
یا دیگر تاسیسات دسترسی‌بالا نمی‌رسد. اما کاملا بهتر از تجربه کار با
مایکروسافت ویندوز خواهد بود.
با پیشرفت لینوکس به‌سمت
\emph{فایل سیستم‌های ژورنالی}\footnote{\texttt{\url{https://en.wikipedia.org/wiki/Journaling_file_system}}}،
\emph{دسترسی سطح بالا}\footnote{\LRE{High availability}}،
...
آموزش (و البته به همان اندازه حمایت برای پروژه‌هایی مانند وب‌سرور آپاچی،
 \LRE{X.org, Freedesktop.org, Tex, LaTeX}
...
\emph{پروژه مستند سازی لینوکس}\footnote{\LRE{Linux Documentation Project}}
و میرورهای آن را می‌شناسند.
به راه‌اندازی یک سایت میرور از \LRE{LDP} نیز فکر کنید.
...
از طریق
{\ttfamily comp.os.linux.announce}, \LRE{LDP}
...
\subsection{پشتیبانی از گنو/لینوکس \LRE{(support)}}
...
کاربران
%\item users
\item \emph{مشاوران}\footnote{\LRE{consultants}}
%\item consultants
\item
...
\emph{روزنامه لینوکس}\footnote{\texttt{\url{http://www.linuxjournal.com/}}}
است. به‌تازگی مجلات دیگری هم پا به عرصه نهاده‌اند.
\emph{فرمت لینوکس}\footnote{\texttt{\url{http://www.linuxformat.com}}}‌(انگلستان)،
\emph{مجله لینوکس}\footnote{\texttt{\url{http://www.linux-magazine.com/}}}
...
با نام
\emph{مجله حرفه‌ای لینوکس}\footnote{\LRE{Linux Pro Magazine}}
شناخته می‌شود.)،
\emph{متن‌باز برای تو}\footnote{\LRE{https://www.opensourceforu.com}}
(هندی و سابقا با نام
\emph{لینوکس برای تو}\footnote{\LRE{LINUX For You}}
شناخته می‌شد.)،
\emph{دایره کامل}\footnote{\texttt{\url{http://fullcirclemagazine.org/}}}
(بین‌المللی بوده و توزیع‌های خانواده اوبونتو را پوشش می‌دهد.)،
\emph{صدای لینوکس}\footnote{\texttt{\url{http://linuxvoice.com/}}}(انگلستان)،
\emph{لینوکس آسان}\footnote{\texttt{\url{http://www.easylinux.de/}}}(آلمان)،
\emph{لینوکس یوزر}\footnote{\texttt{\url{https://www.linux-user.de/}}}(آلمان)،
\emph{اوبونتو یوزر}\footnote{\texttt{\url{http://www.ubuntu-user.com/}}}(شرکت انتشاراتی آلمان واقع در انگلیس)
و
\emph{مجله نرم‌افزار آزاد}\footnote{\texttt{\url{http://freesoftwaremagazine.com/}}}({\itshape صدای متن‌باز} گذشته)
...
انتشارات بهتر شامل
\emph{\url{Linux Weekly News}}\footnote{\texttt{\url{http://lwn.net/}}}،
\emph{\url{DistroWatch Weekly}}\footnote{\texttt{\url{http://distrowatch.com/weekly.php}}}،
\emph{\url{Linux Today}}\footnote{\texttt{\url{http://linuxtoday.com}}}
و
\emph{\url{FreshNews}}\footnote{\texttt{\url{http://www.freshnews.org/}}}
می‌باشند.
...
\emph{پروژه مستندسازی لینوکس}\footnote{\texttt{\url{http://www.tldp.org/}}}
نیز جدید ترین نسخه‌های \LRE{HOWTO}‌ها را برخلاف
توزیع‌های گنو/لینوکس برپایه \LRE{DVD/CD} و غیره دارد.
...
\subsubsection{مشاوران \LRE{(Consultants)}}
%\subsubsection{Consultants}

لاگ‌ها با ایجاد انجمنی که همه می‌توانند درکنار یکدیگر باشند
به مشاوران
(\emph{\LRE{consultant}}\footnote{\LRE{Professional advisor}})
...
شود (چون این رفتار بی‌احترامی و کاملاً ضد اصول گنو/لینوکس است) همچنین
دلیلی هم ووجود ندارد که لاگ در واسطه‌گری بین کاربران نیازمند خدمات مشاوره و متخصصانی که این خدمات حرفه‌ای را ارائه می‌دهند، کمک نکند
...
\footnote{if your consulting advice is poorly heeded and poorly used,
it just might be the case that you need to charge more}
در مقابل انجمن‌های فنی با نظام ارزشی کاملا متفاوت
ذاتا از
\emph{«فرهنگ هدیه»}\footnote{\LRE{Gift culture}}
پیروی می‌کنند.
...
ددرگیری میان دو فرهنگِ کاملاً متفاوتِ ارزش‌محور اجتناب‌ناپذیر است
و ممکن است گاهی ناخوشایند شود.
...
\item
ارائه توضیحاتِ مبهم درباره مشکل، یا توضیحاتی که با گذشت زمان تغییر می‌کنند.
...
\LRE{Oracle, Sybase} و \LRE{DB2}
(و یا
\LRE{VMware, CrossOver Linux}
و موارد مشابه).
...
«\emph{شرکت تسویه وجوه}\footnote{\texttt{\url{https://en.wikipedia.org/wiki/Clearing_house_(finance)}}}»
...
\begin{itemize}
\item
کدام \LRE{ISP} محلی به لینوکس علاقمند است؟
%\item Which local ISP is Linux-friendly?
\item
کدام فروشنده سخت‌افزاری \LRE{PC} لینوکس می‌سازد؟
%\item Are there any local hardware vendors building Linux PCs?
\end{itemize}


نگه‌داری و عمومی‌سازی این نوع اطلاعات نه‌تنها به اعضای لاگ کمکی می‌کند،
بلکه به کسب‌وکارهای علاقمند هم کمک می‌کند و آنها را به ادامه استفاده از
گنو/لینوکس تشویق می‌کند.
این موضوع حتی می‌تواند در برخی موارد به ایجاد محیطی رقابتی کمک کند که
...
\emph{کریس براون}\footnote{\texttt{\url{mailto:cbbrowne@cbbrowne.com}}}
...
یک گروه کاربری می‌تواند اعضا را به مشارکت در پروژه‌های مختلف «توسعه‌ای» ترغیب کند. 
...
اهداف و پروژه‌های وجود دارند که هر یک از اعضای لاگ علاقه‌مند به حمایت از آنها هستند.
...
این بخش نامزد‌های ممکن را فهرست می‌کند. هیچ اولویتی بین آنها وجود ندارد و این فهرست
...
\begin{itemize}
\item
\emph{بنیاد لینوکس}\footnote{\texttt{\url{http://www.linuxfoundation.org/about}}}
%\emph{The Linux Foundation} \texttt{http://www.linuxfoundation.org/aboutadhurl}

\item
\emph{دبیان/نرم‌افزار در منافع عمومی}\footnote{\texttt{\url{http://www.debian.org/donations.html}}}
%\emph{Debian / Software In the Public Interest} \texttt{http://www.debian.org/donations.html}

\item
\emph{بنیاد نرم‌افزار آزاد}\footnote{\texttt{\url{https://my.fsf.org/associate/support_freedom}}}
%\emph{Free Software Foundation} \texttt{https://my.fsf.org/associate/support_freedom}

\item
\emph{پروژه کی‌دی‌ئی}\footnote{\texttt{\url{http://www.kde.org/community/donations/}}}
%\emph{KDE Project (KDE e.V.)} \texttt{http://www.kde.org/community/donations/}

\item
\emph{بنیاد گنوم}\footnote{\texttt{\url{http://www.gnome.org/friends/}}}
%\emph{GNOME Foundation} \texttt{http://www.gnome.org/friends/}

\item
\emph{حفاظت از آزادی نرم‌افزار}\footnote{\texttt{\url{https:/sfconservancy.org/}}}
%\emph{Software Freedom Conservancy} \texttt{https:/sfconservancy.org/}

\item 
\emph{بنیاد موزیلا}\footnote{\texttt{\url{https://foundation.mozilla.org/en/}}}
...
\begin{itemize}
\item
\emph{بنیاد مرزهای الکترونیکی}\footnote{\texttt{\url{http://www.eff.org/}}}

که در سانفرانسیسکو می‌باشد، به اختصار \LRE{EFF} یک
\emph{سازمان عضو محور}\footnote{\texttt{\url{https://en.wikipedia.org/wiki/Membership_organization}}}
...
در میان فعالیت‌های مختلف ما، \LRE{EFF} از ما حمایت می‌کند.
...
\emph{گروه کاربران تک (\LRE{TUG})}\footnote{\texttt{\url{http://www.tug.org/}}}
...
\emph{صحفه کمک‌های مالی انجن تاگ}\footnote{\texttt{\url{https://www.tug.org/donate.html}}}
مراجعه کنید.
%Alternatively, donations can be made 
%\emph{online} \texttt{https://www.tug.org/donate.html}.

\item
\emph{پروژه گوتنبرگ}\footnote{\texttt{\url{http://www.gutenberg.org/}}}

هدف پروژه گوتنبرگ این است که متون کتاب‌های عمومی را به‌طور آزاد و رایگان
...
\item
\emph{پروژه رونبرگ}\footnote{\texttt{\url{http://runeberg.org/}}}

پروژه رونبرگ مانند پروژه گوتنبرگ است. با این تفاوت که بر روی
ارائه ویرایش‌های
\emph{ادبیات کلاسیک اسکاندیناوی}\footnote{\LRE{classic Nordic (Scandinavian) literature}}
...
\item
\emph{بنیاد آموزش متن‌باز}\footnote{\texttt{\url{http://www.osef.org/donations.html}}}
...
آورده شده اند می‌باشد.دو پروژه \LRE{Tux4Kids} و \LRE{OSEF} در کنار هم یک توزیع گنو/لینوکس
(\LRE{Knoppix}
برای کودکان) را بر پایه توزیع
\emph{کلاوس ناپر}\footnote{\LRE{Klaus Knopper}}
که
\emph{ناپیکس}\footnote{\LRE{Knoppix}}
...
\emph{بسته آموزشی برای دانش‌آموزان}\footnote{\LRE{K-12 Box}}
را به‌عنوان یک بسته
\emph{کامپیوتر ورک استیشن پلاگ و پلی}\footnote{\LRE{Plug and Play workstation computer}}
...
با دیدن فایل \LRE{MAINTAINERS} در سورس کد کرنل لینوکس می‌توانید متوجه بشوید.
گنو/لینوکس همچنان به‌صورت بین‌المللی مورد استفاده قرار می‌گیرد.
این افزایش روزافزون
\emph{پایگاه کاربری}\footnote{\LRE{User base}}
...
یک کرنل جدید،
\emph{ری‌کامپایل}\footnote{\LRE{recompiling}}
کردن یک کرنل قدیمی، دست‌وپنجه نرم کردن با یک
\emph{مدیر پنجره}\footnote{\LRE{window manager}}
...
\emph{فرهنگ‌سازی}\footnote{\texttt{\url{https://en.wikipedia.org/wiki/Acculturation}}}
هم معنی می‌کنند).
...
می‌تواند در
\emph{لیست پستی}\footnote{\LRE{mailing list}}،
