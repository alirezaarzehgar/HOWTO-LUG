\section{درباره این مستند}
%\section{About this document}

\subsection{شرایط استفاده}
%\subsection{Terms of use}

حق‌نشر این راهنما از سال ۲۰۰۳ تا ۲۰۲۲ متعلق به
\emph{ریک مون}\footnote{\lr{Rick Moen}}
و از سال ۱۹۹۷ تا ۱۹۹۸ متعلق به
کندال گرانت
\emph{کلارک کندال گرانت}\footnote{\lr{Kendall Grant Clark}}
بوده است.
این مستند می‌تواند تحت مجوز بین‌المللی
\emph{\url{Creative Commons Attribution-ShareAlike 4.0}}\footnote{\texttt{\url{http://creativecommons.org/licenses/by-sa/4.0/}}}
یا نسخه‌های بعدی آن منتشر شود.
%Copyright (C) 2003-2022, Rick Moen.  Copyright (C) 1997-1998 by Kendall Grant 
%Clark. This document may be distributed under the terms set forth 
%in the Creative Commons Attribution-ShareAlike 4.0 International licence at 
%\emph{http://creativecommons.org/licenses/by-sa/4.0/} \texttt{http://creativecommons.org/licenses/by-sa/4.0/}
%, or, at your
%option, any later version.

\subsection{نسخه‌های جدید}
%\subsection{New versions}

نسخه‌های جدید این مستند از وبسایت و منابع مختلف مرتبط به گنو/لینوکس منتشر خواهد شد.
نسخه‌های جدید زبان فارسی آن در مخزن گیتهاب
\url{https://github.com/alirezaarzehgar/HOWTO-LUG}
منتشر خواهد شد.
نسخه‌های انگلیسی آن نیز از سایت‌های
\emph{لینوکس مافیا}\footnote{\texttt{\url{http://linuxmafia.com/lug/}}}
و
\emph{پروژه مستندسازی لینوکس}\footnote{\texttt{\url{http://www.tldp.org/}}}
منتشر خواهند شد.
%New versions of the Linux User Group HOWTO will be periodically
%uploaded to various GNU/Linux Web sites, principally 
%\emph{http://linuxmafia.com/lug/} \texttt{http://linuxmafia.com/lug/}}
%and the
%\emph{Linux Documentation Project} \texttt{http://www.tldp.org/}.

\subsection{لطفا در  بهبود این مستند مشارکت کنید}
%\subsection{Please contribute to this HOWTO}

نسخه ترجمه شده این مستند شدیدا نیازمند مشارکت می‌باشد. لطفا با توسعه و مشارکت در این راهنما
کمک کنید تا راهنمای بهتری را برای ایجاد و نگهداری از لاگ‌ها ایجاد کنیم. در تمام سرفصل‌های این کتاب
کمبود اطلاعات راجع به لاگ‌های داخل ایران وجود دارد. لطفا اگر هر اطلاعاتی دارید در این مستند اضافه کرده
و به مخزن این پروژه در گیتهاب درخواست بدهید. همچنان هر مشکلی از هر جهتی در این پروژه وجود دارد
می‌توانید با درست کردن ایشو در مخزن گیتهاب
\emph{این پروژه}\footnote{\texttt{\url{https://github.com/alirezaarzehgar/HOWTO-LUG}}}
به توسعه آن کمک کنید.
از هر نوع مشارکتی در این پروژه استقبال می‌شود. اگر تمایل دارید این پروژه را بهبود بدهید.

در صورتی که سوالی بپرسید یا بازخوردی را نیز می‌خواستید به مدیر فعلی نسخه زبان اصلی این مستند بگویید،
می‌توانید از آدرس
\href{mailto:rick@linuxmafia.com}{rick@linuxmafia.com}
او را در جریان بگذارید.

%I welcome questions about and feedback on this document. Please send
%them to me at \ifpdf
%\href{mailto:rick@linuxmafia.com}{rick@linuxmafia.com}%
%\else
%\onlynameurl{rick@linuxmafia.com}%
%\fi{}
%. {\itshape I am especially interested in
%hearing from LUG leaders around the world, especially outside the USA\/}.
%Please let me know of innovative or noteworthy things your group does,
%meriting description here.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Documention history was ignored in the PDF version of this guide
%\input{sections/history}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{تقدیر و تشکر}
%\subsection{Acknowledgements}

از
\emph{کندال گرانت کلارک}\footnote{\lr{Kendall Grant Clark}}
برای نوشتن اولین نسخه این مستند در سال ۱۹۹۷-۱۹۹۸ و اعتماد کردن به من برای گرفتن و بازسازی
اثر او در سال ۲۰۰۳ بسیار متشکرم.
%I would like to give a big thank-you to Kendall Grant Clark for the 
%initial versions of this document in 1997-1998, and for trusting me to take
%over and renovate his creation starting in 2003.

تشکری صمیمانه از
\emph{کریس براون}\footnote{\lr{Chris Browne}}
بابت توضیحات او راجع به گروه‌های غیرانتفاعی و خیریه در کانادا و نظراتش راجع به کمک‌های مالی به‌عنوان
راهی برای مشارکت در گنو/لینوکس و جنبش نرم‌افزار آزاد/متن‌باز و ایده‌های او راجع به انواع مشکلات قانونی
و سیاسی که ممکن است در لاگ‌ها اتفاق بی‌افتند.
%Warm regards and thanks to 
%\emph{Chris Browne} \texttt{mailto:cbbrowne@cbbrowne.com}
% for describing the situation with
%non-profit and charitable groups in Canada, his thoughts on financial
%donations as a way to participate in GNU/Linux and the free software and 
%open-source software movements, and his ideas about the kinds of
%political issues likely to arise within LUGs.

به‌علاوه افراد زیر نیز نظرات و پیشنهادات کمک کننده و مفید بسیاری داشتند و از آنها نیز متشکریم:
%In addition, the following people have made helpful comments and
%suggestions:

\begin{itemize}
\item \emph{متیو کریگ}\footnote{\lr{Matthew Craig}}
\item \emph{جف گارواس}\footnote{\lr{Jeff Garvas}}
\item \emph{گرگ‌هانکینز}\footnote{\lr{Greg Hankins}}
\item \emph{جیمز هرتزلر}\footnote{\lr{James Hertzler}}
\item \emph{توماس کاپلز}\footnote{\lr{Thomas Kappler}}
\item \emph{مارتین کارلسون}\footnote{\lr{Martin Karlsson}}
\item \emph{هوگو ون در کویج}\footnote{\lr{Hugo van der Kooij}}
\item \emph{روهیت کومار}\footnote{\lr{Rohit Kumar}}
\item \emph{دیوید لویر}\footnote{\lr{David Lawyer}}
\item \emph{چارلز لیندال}\footnote{\lr{Charles Lindahl}}
\item \emph{دون مارتی}\footnote{\lr{Don Marti}}
\item \emph{وینچنزو ویرجیلیو}\footnote{\lr{Vincenzo Virgilio}}
\end{itemize}

